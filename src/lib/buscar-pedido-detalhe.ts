...
export async function buscarPedidoDetalhe(
  ...
  const [{ data: pedido }, { data: parcelas }] = await Promise.all([
    supabase
      .from("pedidos")
      .select(
        "*, clientes(nome, cpf_cnpj, endereco, bairro, cidade, uf), pedido_itens(quantidade, preco_unitario, produtos(nome, codigo_interno)), pedido_pagamentos_mistos(forma_pagamento, valor)",
      )
      .eq("id", id)
      .single(),
    supabase
