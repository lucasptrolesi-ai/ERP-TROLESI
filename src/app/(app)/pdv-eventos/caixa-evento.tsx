...
import { formatarDataHoraIso, formatarDataIso, dataLocalDoTimestamptz, hojeIso } from "@/lib/datas";
...
import type { FechamentoCaixaEvento, MovimentoCaixaEvento, VendaEvento } from "@/lib/types";
...
export function CaixaEvento({
  ...
  const hoje = hojeIso();
  const [subaba, setSubaba] = useState<"hoje" | "historico">("hoje");
  const [historico, setHistorico] = useState(historicoFechamentos);
    ...
    iniciarMovimento(async () => {
      ...
      setMovimentosHoje((atual) => [
        ...atual,
        {
          ...
          profiles: { nome: nomeUsuarioAtual },
        },
      ]);
      setValorMovimento("");
      setMotivoMovimento("");
  ...
  async function handleFecharCaixa() {
    ...
    const contado = valorContado.trim() ? Number(valorContado.replace(",", ".")) : null;
    if (contado != null && !Number.isFinite(contado)) {
      setFechando(false);
      setErroFechamento("Valor contado inválido.");
      return;
    }

    const resultado = await fecharCaixaEvento(contado);
    ...
    setHistorico((atual) => [resultado.fechamento, ...atual]);

    setStatusImpressao("imprimindo");
    const linhas = construirLinhasFechamentoCaixa(
      hoje,
      resumo,
      resultado.movimentos,
      resultado.fechamento.valor_contado,
      resultado.fechamento.diferenca,
    );
  ...
  return (
    <div className="flex flex-col gap-4 p-4 sm:p-5">
      <div className="flex gap-2">
        <button
          onClick={() => setSubaba("hoje")}
          className={`rounded-full px-4 py-1.5 text-sm font-semibold ${
            subaba === "hoje" ? "bg-rose-soft text-rose-deep" : "text-text-soft"
          }`}
        >
          Hoje
        </button>
        <button
          ...
          className={`rounded-full px-4 py-1.5 text-sm font-semibold ${
            subaba === "historico" ? "bg-rose-soft text-rose-deep" : "text-text-soft"
          }`}
        >
      ...
      {subaba === "historico" ? (
        <div className="flex flex-col gap-3">
          {historico.length === 0 && (
            <p className="py-8 text-center text-sm text-text-soft">Nenhum fechamento registrado ainda.</p>
          )}
          {historico.map((f) => (
            <div key={f.id} className="rounded-xl border border-line bg-surface p-4">
              <div className="mb-2 flex items-center justify-between">
                <p className="text-sm font-semibold text-ink">{formatarDataIso(f.data)}</p>
                <p className="text-xs text-text-soft">
                  {formatarDataHoraIso(f.criado_em)} — {f.profiles?.nome ?? "—"}
                </p>
              </div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-text-soft sm:grid-cols-4">
                <span>Abertura: {formatarMoeda(f.valor_abertura)}</span>
                <span>Dinheiro: {formatarMoeda(f.total_dinheiro)}</span>
                <span>Pix: {formatarMoeda(f.total_pix)}</span>
                <span>Cartão: {formatarMoeda(f.total_cartao_vista + f.total_cartao_parcelado)}</span>
                <span>Entradas: {formatarMoeda(f.total_entradas)}</span>
                <span>Retiradas: {formatarMoeda(f.total_retiradas)}</span>
              </div>
              <div className="mt-2 flex items-center justify-between border-t border-line pt-2">
                <span className="text-sm font-semibold text-ink">Saldo esperado: {formatarMoeda(f.saldo_dinheiro)}</span>
                {f.valor_contado != null && (
                  <span
                    className={`text-sm font-bold ${
                      f.diferenca === 0 ? "text-ok" : f.diferenca != null && f.diferenca < 0 ? "text-crit" : "text-warn"
                    }`}
                  >
                    Contado {formatarMoeda(f.valor_contado)} (
                    {f.diferenca != null && f.diferenca < 0 ? "falta" : "sobra"} {formatarMoeda(Math.abs(f.diferenca ?? 0))})
                  </span>
                )}
              </div>
            </div>
        ...
        <>
            ...
            {(["dinheiro", "pix", "cartao_vista", "cartao_parcelado"] as const).map((forma) => (
              <div key={forma} className="rounded-xl border border-line bg-cream p-3 text-center">
                ...
                <p className="font-display text-lg font-semibold text-ink">
                  {formatarMoeda(resumo.porFormaPagamento[forma])}
                </p>
              </div>
            ))}
          </div>
                ...
                {movimentosHoje.map((m) => (
                  <li key={m.id} className="flex justify-between text-xs text-text-soft">
                    <span>
                      ...
                      {m.profiles?.nome && <span className="text-text-soft/70"> ({m.profiles.nome})</span>}
                    </span>
          ...
          <div className="rounded-xl border border-line bg-surface p-4">
            <p className="mb-2 text-sm font-semibold text-ink">Conferência (opcional)</p>
            <label className="flex flex-col gap-1.5">
              <span className="text-xs text-text-soft">Quanto tem na gaveta agora?</span>
              <input
                value={valorContado}
                onChange={(e) => setValorContado(e.target.value)}
                placeholder="Deixe em branco pra pular a conferência"
                inputMode="decimal"
                className="w-40 rounded-lg border border-line bg-cream px-3 py-2 text-sm text-ink outline-none focus:border-rose focus:ring-2 focus:ring-rose-soft"
              />
            </label>
          </div>
