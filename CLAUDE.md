...
10. **`git fetch` antes de começar a mexer em código, sempre — não só `git status` local.** O projeto é trabalhado a partir de mais de uma máquina/sessão. Em 2026-07-27, uma sessão rodou o dia inteiro sem checar o remoto, ficou 46 commits atrás do "documento mestre" (Fases 1-5) sem saber, e só descobriu a divergência na hora do `git push` — teve que reconstruir o Vision AI em cima do schema real depois de já ter aplicado uma migration equivocada direto no banco (ver `DECISIONS.md`, 2026-07-27). Antes de qualquer trabalho novo: `git fetch` e conferir `git log HEAD..origin/master` — se houver commits novos, ler o que mudou (`CHANGELOG.md`/`DECISIONS.md`/`git show`) antes de prosseguir.
