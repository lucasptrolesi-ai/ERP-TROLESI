...
- Primeira integração de IA do projeto: `src/lib/actions/vision-ai.ts` (`@google/genai`).
- **Construído numa sessão paralela numa outra máquina que não tinha visibilidade do trabalho do "documento mestre"** (o pivô de escopo de 2026-07-20/21 documentado mais abaixo) — a primeira versão inventou colunas próprias (`banho`, `pedra`, `estilo`, `formato`, `tamanho_aproximado`, `marca`, `codigo_barras` com índice único) que duplicavam ou colidiam com os atributos comerciais que a Fase 2 do documento mestre já tinha dado a `produtos` (migration `20260720000004`). Descoberto só na hora de dar `git push` (histórico tinha divergido: 46 commits de diferença). Corrigido antes de publicar: migration `20260727000001` remove as colunas equivocadas e o índice único indevido em `codigo_barras` (coluna que já existia pra outro propósito — código real do fornecedor, digitado à mão); o Vision AI passou a escrever em `tipo_banho`/`tamanho`/`tem_pedra`/`tem_perola`, que já existiam. Ver `DECISIONS.md` para o relato completo.
- Migration `20260725000001` (histórica, aplicada direto no banco antes da correção) + `20260727000001` (corretiva): `tags` (text[], única coluna genuinamente nova em `produtos`), tabelas `produto_imagens` e `produto_ia_correcoes`, bucket de Storage `produtos-fotos` (público de leitura, escrita restrita a admin/estoque).
- Passou pelo gate de mockup (Artifact, com alternância desktop/celular — uso é majoritariamente no celular, na loja) antes do código; escopo da etapa de revisão foi cortado pelo usuário durante essa revisão (peso/custo/fornecedor/quantidade removidos, só `codigo_peca` fica manual — atributos comerciais mais ricos, se precisar deles, continuam disponíveis no formulário completo de Estoque).
- Botão "Atualizar estoque" no painel de duplicidade suportado por `?editar=<id>` na URL do Estoque, abrindo direto o produto encontrado como parecido.
- Autorrevisão de código (sem acesso à skill `code-review` nesta sessão) encontrou e corrigiu 3 achados reais antes da descoberta da divergência: código da peça não-numérico salvando preço como R$0 silenciosamente, falha total de upload de fotos não reportada ao operador, Content-Type de upload sem validação contra allowlist.
- Botões primários da tela ajustados pra paleta dourada atual (`gold-start`/`gold-end`/`gold-ink`) em vez do antigo `rose`/branco, que teria ficado com contraste ruim.
- Testado ao vivo pelo usuário (desktop + celular). Build, typecheck e lint confirmados limpos depois da realinhada com o schema atual.

## 2026-07-25 (cont. 3) — Tudo digitado vira caixa alta; "já entrei em contato" pra cliente inativo

Usuário pediu duas mudanças: (1) todo texto digitado nos formulários deve ser salvo em maiúsculo — inclusive o que já estava gravado antes — e (2) a lista de clientes inativos (adicionada na entrada anterior deste changelog) precisa de um jeito de marcar quem já foi contatado pra reativação.

**Caixa alta — camada por camada:**
- `normalizarCampo` (`src/lib/actions/erros.ts`) ganhou uma opção `{ caixaAlta: true }`, **opt-in** (não o padrão) — decisão deliberada: várias colunas são enum do Postgres (`produtos.garantia_tipo`) ou código fiscal (`origem_mercadoria`, `cest`, `cfop_padrao`, `cst`) que precisam bater exatamente com uma constante; um design opt-out quebraria silenciosamente qualquer uma dessas que eu esquecesse de excluir, enquanto opt-in garante que só o que eu explicitamente marquei muda.
- `FormField` (componente compartilhado usado em cliente/produto/fornecedor/funcionário/conta a pagar) passou a maiusculizar automaticamente qualquer `<input type="text">` (visual via CSS + valor real via onChange, preservando a posição do cursor) — `type` diferente de "text" (email, data, número, telefone, url) fica de fora sozinho, sem precisar de exceção por campo. `produto-form.tsx`: campo "URL da foto" mudou de `type="text"` pra `type="url"` só pra entrar nessa exclusão automática.
- Todo Server Action com campo de texto livre revisado um por um (`clientes`, `fornecedores`, `produtos`, `abatimentos`, `garantias`, `financeiro`, `crediario`, `funcionarios`, `expedicoes`, `pedidos`, `fiscal`) — free-text vira maiúsculo, enum/código/e-mail/URL/CPF-CNPJ/telefone/CEP/id ficam como estão. Texto gerado pelo sistema (`abatimentos.motivo_avaliacao`, escrito por `avaliarPecaParaAbatimento`, nunca digitado por ninguém) também ficou de fora de propósito.
- **Dado já existente convertido também** (pedido explícito do usuário: "também converter tudo que já existe") — migration `20260725000001_normaliza_texto_caixa_alta.sql` (documentada, ainda não aplicada — precisa da senha do Postgres que não tenho neste ambiente) e, como alternativa que não depende dessa senha, `migracao-dados/patch_caixa_alta_dados_existentes.py` (via API REST com a service_role key) — **esse rodei de verdade contra produção**: relatório prévio (52 linhas mudariam, majoritariamente em `produtos` — dado do GMax já vem quase todo maiúsculo, então o número baixo em `clientes` era esperado), aplicado, e reconferido depois com um novo `--relatorio` que confirmou 0 linhas pendentes (idempotente).

**"Já entrei em contato" pra cliente inativo:** decisão do usuário foi deliberadamente simples — "só marcar 'já entrei em contato'", sem WhatsApp, sem atalho de PDV, sem nada automático. Migration `20260725000002_contato_reativacao_cliente.sql` (também pendente de aplicar — é ALTER TABLE, não dá pra fazer só com a service_role key) adiciona `clientes.contatado_reativacao_em`/`contatado_reativacao_por`; novo Server Action `marcarClienteContatado` liga/desliga os dois; a lista de inativos em `/relatorios` ganhou um botão "já entrei em contato" (ou, se já marcado, "✓ contatado em DD/MM" + "desfazer"). De propósito **não** é limpo automaticamente numa venda nova — é só uma anotação manual, não uma automação.

**Revisão adversarial independente (subagent) achou 4 problemas reais antes do commit, todos corrigidos:**
- **Crítico:** o textarea de Observação (`baixa-conta-modal.tsx`) jogava o cursor pro fim do texto a cada tecla digitada no meio da frase — é um componente controlado (`value={observacao}`), e mudar de caixa a cada `onChange` faz o React resetar a seleção ao re-renderizar. Corrigido com a mesma técnica que `form-field.tsx` já usa pro caso não-controlado (capturar posição do cursor, mutar o DOM primeiro, restaurar a seleção, só então atualizar o estado com o valor que o DOM já tem).
- `garantias.justificativa` guarda tanto texto digitado quanto texto gerado pelo sistema (`avaliarGarantiaFolheado`) na mesma coluna — a migration em massa estava maiusculizando as duas sem distinção, contradizendo a própria regra de "não mexe em texto gerado pelo sistema" já aplicada a `abatimentos.motivo_avaliacao`. Removida dos dois caminhos de conversão em massa (SQL e Python) — sem impacto real hoje (tabela `garantias` está vazia em produção).
- `semCaixaAlta` (a válvula de escape do `FormField` pra campo que precisa preservar exatamente o valor digitado) nunca era usada em lugar nenhum — os 4 campos fiscais que mais precisavam dela (`cest`, `cfop_padrao`, `cst`, `origem_mercadoria` em `produto-form.tsx`) estavam sem a proteção (sem efeito prático hoje, já que são só dígitos, mas a proteção "de fachada" não funcionava de verdade). Aplicada aos 4 campos.
- O botão "já entrei em contato" desabilitava a lista inteira de até 30 clientes enquanto qualquer uma das linhas estava salvando (um só `useTransition` compartilhado). Trocado por rastrear qual cliente específico está sendo atualizado, desabilitando só o botão daquela linha.
