...
| 4 — Regras especiais | ✅ Concluída (schema+regra+UI mínima) | Abatimento e garantias com UI funcional; crediário legado/comissões/frete só schema+regra por ora (UI dedicada é próximo passo) |
| 5 — Relatórios e qualidade | 🔶 Em andamento | Code-review parcial (2/10 ângulos rodaram, 5 bugs reais corrigidos); relatórios/dashboards de indicadores ainda não construídos; ver pendências abaixo |
...
### Fase 4 — Regras especiais (concluída — schema+regra+UI mínima, 2026-07-21)
...
- `converter_cliente_em_crediario`: só admin, justificativa obrigatória, auditado.
- UI funcional pros dois fluxos mais novos: `/abatimentos` (avaliação com prévia de elegibilidade em tempo real, aprovar/reprovar) e `/garantias` (formulário por tipo, prévia de aprovação pra folheado a ouro, decisão manual pra Orient/autenticidade). **Crediário/comissões/frete ficam só com schema+regra — UI dedicada é trabalho futuro.**
- **Exceção registrada ao gate de mockup** (regra 1 do `CLAUDE.md`): as telas de Abatimentos/Garantias foram construídas direto em código, sem Artifact/preview prévio, sob autorização explícita do usuário (`/goal` autônomo, "não precisa me perguntar nada"). Não é precedente — próximas telas novas com liberdade de design real voltam a exigir mockup.
...
- Relatórios/indicadores (seção 24 do documento mestre: vendas, descontos, abatimentos, garantias, primeira compra, clientes reativados/inativos, crediários, atrasos, parcelamentos, taxas de cartão, fretes grátis, estoque, comissões, cancelamentos) — nenhum construído ainda.
- UI dedicada de crediário legado, comissões e frete/expedição.
...
- **UI de crediário legado, comissões e frete/expedição** — schema e regra prontos, telas ainda não.
- **Relatórios/indicadores** (Fase 5) — nenhum construído ainda.
...
Retomar o code-review completo (8 ângulos pendentes) assim que o limite de sessão resetar, aplicar qualquer achado real, e então seguir pra Fase 5: relatórios/indicadores e UI de crediário/comissões/frete.
