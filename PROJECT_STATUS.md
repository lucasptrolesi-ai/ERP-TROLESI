...
## 2026-07-22 (cont. 2) — Verificação final de dado sempre atualizado

Pedido direto do usuário pra "tudo atualizado para todos a todo momento". Auditoria de todas as 17 Server Actions (`src/lib/actions/`) cruzando toda mutação contra toda leitura da mesma tabela em outra rota. Achados reais corrigidos: bug financeiro (`ajustar_valor_pedido` desalinhava `contas_receber` numa venda cartão/promissória 1x — e a 1ª versão do fix tinha um furo de RLS que o deixava sem efeito pro papel vendedor, achado por revisão independente e corrigido isolando a checagem numa function `security definer`), mais 5 lacunas de `revalidatePath` (funcionários não atualizavam `/permissoes`/`/comissoes`, clientes não atualizavam `/abatimentos`/`/crediario`/`/garantias`/`/relatorios`, fornecedores não atualizavam `/financeiro`). Novo: Supabase Realtime habilitado em `produtos`/`pedidos`/`pedido_itens`/`contas_receber`/`contas_pagar` — um terminal fechando venda ou dando baixa atualiza a tela de outro terminal sozinho (`src/components/realtime-refresh.tsx`, montado no `AppShell`). Detalhes completos em `CHANGELOG.md`/`DECISIONS.md` de hoje.

**Pendente de você:** as migrations `20260722000005` e `20260722000006` ainda não foram aplicadas no banco real (preciso da conexão com o pooler ou que você rode o SQL direto no SQL Editor do Supabase — não tenho a senha do banco guardada neste ambiente).

## 2026-07-22 — Deploy publicado, uso real começou

O site está publicado (`https://erp-trolesi.vercel.app`) e o usuário está testando/usando com dados reais, gerando correções e pedidos em tempo real em vez de trabalho especulativo. Nesta data: senha recuperada e fluxo de recuperação construído, gestão completa de funcionários pelo app, estoque negativo autorizado, 38 pedidos reais de maio-julho reconciliados contra o Firebird do GMax, paleta rebrand (navy/café + dourado), Financeiro de volta ao menu (com botão de extornar direto na lista), e um cupom térmico de 58mm com impressão nítida via ESC/POS — que exigiu descobrir em campo que a loja fecha venda de múltiplos aparelhos (não só uma máquina fixa), corrigindo a arquitetura pra uma fila via Supabase em vez de um agente local por loopback. Ver `CHANGELOG.md`/`DECISIONS.md` de 2026-07-22 pros detalhes técnicos e `print-agent/README.md` pra como o print-agent funciona/se instala numa máquina nova.
