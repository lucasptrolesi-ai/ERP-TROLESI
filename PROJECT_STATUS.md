...
| 4 — Implementação módulo a módulo | ✅ Concluída (2026-07-16) | Cadastros, Estoque, Pedidos, Financeiro e **Fiscal** (modo conferência — gera XML/DANFE de rascunho, nada transmitido à SEFAZ) concluídos. Ordem seguida: Cadastros → Estoque → Pedidos → Financeiro → Fiscal. Ver detalhes abaixo |
...
| 6 — Conferência fiscal (XML vs. GMax) | ⏳ Não iniciada | Comparar nota a nota o XML gerado pelo módulo Fiscal com o que o GMax emite, validar no validador Sintegra do usuário |
| 7 — Deploy + liberação da emissão fiscal real | ⏳ Não iniciada | Requer autorização explícita e escolha de provedor de NF-e (Focus NFe/eNotas — usuário decidiu adiar essa escolha) |

**Fora das 7 fases originais, construído a pedido do usuário (2026-07-15):** Dashboard operacional (KPIs, pedidos recentes, estoque em alerta), fechamento de caixa diário/semanal/mensal (aba nova em Financeiro) e alerta de meta de faturamento mensal (R$55 mil). Ver detalhes abaixo.

## Controle de versão

Repositório git inicializado em 2026-07-13 (`erp-trolesi-novo/.git`). Histórico local com todos os módulos até aqui (Cadastros, Estoque, Pedidos, Financeiro, Fase 5, Dashboard/Fechamento, Fiscal). **Nenhum remoto configurado ainda** — o usuário pediu (2026-07-16) para subir tudo num repositório GitHub próprio (pra poder clonar em outro PC); aguardando ele criar o repositório e enviar a URL.
...
- **Push para o GitHub**: usuário pediu (2026-07-16) pra subir o repositório inteiro num remoto GitHub próprio, pra poder clonar em outro PC — aguardando ele criar o repositório e enviar a URL.
...
- **Provedor de NF-e** (Focus NFe/eNotas) ainda não escolhido — bloqueia só a Fase 7 (emissão real), não o módulo Fiscal em modo conferência, que já está funcionando.

## Próxima tarefa

**Push para o GitHub** (bloqueado, aguardando URL do usuário), depois **Fase 6 — Conferência fiscal**: comparar XML/DANFE gerado pelo módulo Fiscal com o que o GMax emite pras mesmas vendas reais, validar no validador Sintegra do usuário, até bater 100%.
