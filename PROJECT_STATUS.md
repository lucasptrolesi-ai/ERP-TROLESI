...
| 5 — Relatórios e qualidade | 🔶 Em andamento | `/relatorios` construído (faturamento, vendas, descontos por forma de pagamento, abatimentos/garantias do período, estoque baixo) — indicadores restantes (primeira compra, clientes reativados, taxas de cartão, fretes grátis) pendentes. Code-review parcial (2/10 ângulos rodaram, 5 bugs reais corrigidos); ver pendências abaixo |
...
- **Limitações conhecidas, não escondidas:** geração automática de comissão por evento (venda/recebimento/fechamento mensal) ainda não existe — só lançamento manual; frete grátis não é aplicado automaticamente (pendência #2 do documento mestre, base de cálculo em aberto) — o campo existe, é marcado manualmente com motivo.

### Fase 5 — Relatórios e qualidade (em andamento)

- `/relatorios` (só admin): faturamento + variação vs. período anterior, vendas/ticket médio/cancelamentos, quebra por forma de pagamento, abatimentos e garantias aprovados/reprovados no período, estoque abaixo do mínimo — reaproveita os helpers de período já existentes (`src/lib/relatorios.ts`).

**Code-review rodado sobre as Fases 1-4** (`/code-review xhigh`, 10 ângulos): 8 dos 10 agentes pararam por limite de sessão da conta antes de terminar (retomam quando o limite resetar). Os 2 que completaram (Reuso, Wrapper/Proxy) encontraram achados reais, todos corrigidos e commitados:
1. `aprovarAbatimento`/`reprovarAbatimento` e a decisão de garantia faziam UPDATE direto, sem checar a permissão granular específica nem gravar em `audit_log` — corrigido com 3 functions SQL novas (`aprovar_abatimento`, `reprovar_abatimento`, `aprovar_reprovar_garantia`).
2. `extornar_pedido`/`ajustar_valor_pedido` não bloqueavam pedido já `lancado_gmax` — corrigido.
3. `extornar_pedido` não limpava `parcelas_planejadas`, permitindo reimprimir promissórias de venda cancelada — corrigido (limpa no extorno + guarda defensiva na página de impressão).
4. (achado à parte, não do review) `percentual_descascamento` de garantia usava `|| null`, tratando 0% como campo vazio — corrigido.
5. Reuso: `parseMoeda` já existente não estava sendo usado em `abatimentos.ts`/`garantias.ts` — corrigido.

**Ainda não feito:**
- Os 8 ângulos de review que falharam por limite de sessão (line-by-line, removed-behavior, cross-file tracer, language-pitfall, simplification, efficiency, altitude, conventions) — retomar quando possível, não é seguro assumir que não há mais achados.
- Indicadores restantes da seção 24 do documento mestre não cobertos pelo `/relatorios` ainda: primeira compra, clientes reativados/inativos, crediários/atrasos, parcelamentos detalhado, taxas de cartão, fretes grátis, comissões (o relatório atual cobre vendas/descontos/abatimentos/garantias/estoque).
- Geração automática de comissão por evento (venda/recebimento/fechamento mensal) — só lançamento manual por ora.
- Aplicação automática de frete grátis — pendência #2 (base de cálculo) segue em aberto, campo é manual.
- Testes de integração contra banco real (as regras implementadas em SQL — primeira compra/reativação, idempotência, conversão de crediário — têm teste `it.todo` registrado em `src/lib/regras-comerciais.pendente.test.ts`, mas nenhum teste de integração de verdade ainda, só as funções puras extraídas têm cobertura Vitest real).
- Segurança/performance/responsividade/acessibilidade: revisão dedicada ainda não feita além do que o code-review parcial cobriu.

## Pendências reais

- **Continuar o code-review** (8/10 ângulos pendentes por limite de sessão) antes de considerar as Fases 1-4 realmente fechadas.
- **Indicadores restantes de relatórios** (primeira compra, reativação, crediário/atrasos, taxas de cartão, fretes grátis, comissões).
...
- **11 ambiguidades do documento mestre** registradas em `pending_decisions` (banco real), aguardando decisão humana — nenhuma regra que dependa delas foi implementada por suposição.

## Próxima tarefa

Retomar o code-review completo (8 ângulos pendentes) assim que o limite de sessão resetar, aplicar qualquer achado real, e então completar os indicadores restantes de `/relatorios` e a geração automática de comissão.
