...
| 5 — Relatórios e qualidade | 🔶 Em andamento | `/relatorios` construído (faturamento, vendas, descontos por forma de pagamento, abatimentos/garantias do período, estoque baixo) — indicadores restantes (primeira compra, clientes reativados, taxas de cartão, fretes grátis) pendentes. **Code-review completo (10/10 ângulos), vários achados reais corrigidos incluindo 1 crítico** — ver detalhes abaixo |
...
- Segurança/performance/responsividade/acessibilidade: sem uma revisão dedicada além do que o code-review de 10 ângulos já cobriu.
- Não existe ainda UI pra conceder permissões granulares (`permissoes_usuario`) — só admin tem acesso de fato às ações que exigem permissão específica (aprovar abatimento/garantia, conceder frete grátis etc.).

## Pendências reais

- **Indicadores restantes de relatórios** (primeira compra, reativação, crediário/atrasos, taxas de cartão, fretes grátis, comissões).
- **UI de concessão de permissões granulares** — hoje só admin consegue exercer as ações protegidas por `tem_permissao()`.
...
Completar os indicadores restantes de `/relatorios`, a geração automática de comissão por evento, e (se o usuário quiser) uma tela de concessão de permissões granulares pra destravar as ações hoje restritas a admin.
