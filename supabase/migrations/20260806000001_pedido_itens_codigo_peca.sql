-- Guarda o código da peça usado em cada linha do pedido (pedido direto do
-- usuário, 2026-08-06): até aqui só o preço final calculado
-- (codigo_peca × multiplicador) era gravado — o código em si nunca
-- persistia, só existia no carrinho durante a venda. Sem isso, não dava
-- pra mostrar no cupom/detalhe do pedido o código que o vendedor realmente
-- digitou (cada peça física pode ter um código diferente, mesmo sendo "o
-- mesmo produto" no cadastro — por isso o carrinho não soma quantidade na
-- mesma linha, ver migration anterior de linha_id).
--
-- ROLLBACK:
-- alter table public.pedido_itens drop column if exists codigo_peca;
-- (reverter pra versão v6 de criar_pedido, ver
-- 20260721000012_criar_pedido_v6_comissao_automatica.sql)

alter table public.pedido_itens add column codigo_peca numeric(10, 2);
  ...
  if p_status in ('faturado', 'aguardando_lancamento_gmax') then
    ...
    if v_motivo_minimo is not null and v_total < v_minimo then
      if p_excecao_justificativa is null or length(trim(p_excecao_justificativa)) = 0 then
        raise exception 'Venda de % exige mínimo de % (valor atual: %). Informe uma justificativa de exceção autorizada pra prosseguir abaixo do mínimo.',
