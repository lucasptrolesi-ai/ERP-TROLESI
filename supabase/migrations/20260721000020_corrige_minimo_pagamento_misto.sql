-- Correção de achado do autorrevisão desta leva (não dá pra rodar a skill
-- `/code-review` de dentro do agente — ela só roda por comando explícito do
-- usuário — então a revisão desta leva grande foi manual): a v7 de
-- criar_pedido (20260721000018) checava só `jsonb_array_length(...) = 0`
-- pra "pagamento misto precisa de pelo menos DUAS formas" — ou seja, a
-- mensagem prometia 2, mas o código só exigia 1 (array não vazio). Um
-- "pagamento misto" com uma perna só não faz sentido de negócio (é só a
-- forma direta) e a validação client-side (novo-pedido.tsx) já bloqueia
-- isso, mas o servidor é quem precisa garantir de verdade — mesmo
-- princípio já aplicado em todo o resto do projeto (nunca confiar só na
-- checagem client-side).
--
-- ROLLBACK: reverter a checagem pra `jsonb_array_length(p_pagamentos_mistos) = 0`
...
begin
  ...
  if p_forma_pagamento = 'misto' and coalesce(jsonb_array_length(p_pagamentos_mistos), 0) < 2 then
