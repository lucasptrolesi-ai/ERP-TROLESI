-- Corrige achado de code review (2026-08-11, ver DECISIONS.md): venda com
-- pagamento misto onde uma das pernas é promissória salvava com sucesso,
-- mas a dívida daquela perna nunca virava `contas_receber` — `criar_pedido`
-- só sabia gerar parcelas quando a forma era promissória "pura" ou cartão
-- parcelado, e a validação de soma comparava contra o total inteiro do
-- pedido (errado pra um misto, onde só uma fração é promissória).
--
-- Agora: quando `p_forma_pagamento = 'misto'` e há parcelas, o valor
-- esperado das parcelas passa a ser a soma das pernas de
-- `p_pagamentos_mistos` com forma 'promissoria' (não mais `v_total`), e as
-- linhas de `contas_receber` geradas nesse caso são gravadas com
-- forma_pagamento = 'promissoria' (não 'misto', que não faz sentido como
-- forma de uma conta a receber individual).
--
-- ROLLBACK:
...
-- (recriar a partir de 20260810000001_restaura_pagamento_misto_com_codigo_peca.sql se precisar reverter)
