-- Fase 5: pagamento misto (seção 16, decisão registrada em pending_decisions
-- pra 'desconto_pagamento_misto') — última forma de pagamento do documento
-- mestre que faltava existir. Sem desconto automático quando misto (mesmo
-- tratamento que cartão de crédito/promissória já têm) — o próprio
-- documento veta inventar uma divisão proporcional sem regra definida, e
-- essa é a leitura mais segura disso. Cada perna é registrada separada
-- (seção 16: "registrar cada recebimento separadamente"), com soma
-- validada contra o total no servidor.
--
-- ATENÇÃO (lição já registrada neste projeto): adicionar um parâmetro novo
-- via CREATE OR REPLACE cria um overload duplicado em vez de substituir —
-- por isso o DROP explícito da assinatura de 12 parâmetros antes de criar a
-- de 13.
--
-- Depende do valor 'misto' do enum forma_pagamento já commitado por
-- 20260721000017_pagamento_misto_enum.sql (migration separada de propósito
-- — ver o comentário lá).
--
-- ROLLBACK:
...
-- (recriar a versão de 12 parâmetros a partir de 20260721000012 se precisar reverter)
-- drop table if exists public.pedido_pagamentos_mistos;

create table public.pedido_pagamentos_mistos (
...
alter table public.pedido_pagamentos_mistos enable row level security;

create policy "time logado lê pedido_pagamentos_mistos"
  on public.pedido_pagamentos_mistos for select
