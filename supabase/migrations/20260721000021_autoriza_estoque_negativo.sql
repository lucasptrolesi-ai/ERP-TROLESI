-- Fase 5 (pós-entrega): a pedido do usuário, vendas deixam de ser
-- bloqueadas por estoque insuficiente — o estoque pode ficar negativo
-- (venda registrada antes da contagem/reposição física ser atualizada no
-- sistema, prática comum na loja). O produto continua precisando existir
-- (não dá pra vender um produto_id inválido), só o limite de quantidade
-- disponível deixou de ser uma trava.
--
-- ROLLBACK: reverter pra 20260721000020 (restaura o bloqueio de estoque insuficiente)
...
begin
  ...
  loop
    ...
    if p_status = 'faturado' then
      ...
      -- Estoque negativo autorizado (pedido do usuário, 2026-07-21): venda
      -- não é mais bloqueada por falta de estoque contado no sistema — a
      -- baixa acontece do mesmo jeito, mesmo que o saldo fique negativo.
