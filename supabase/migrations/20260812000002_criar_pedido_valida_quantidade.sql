-- Defesa em profundidade: `criar_pedido` validava preco_unitario < 0 mas
-- nunca quantidade <= 0 (achado de code review, 2026-08-11 — ver
-- DECISIONS.md). Não é explorável pela tela hoje (novo-pedido.tsx já trava
-- em Math.max(1, …) client-side), mas uma quantidade negativa reduziria o
-- subtotal E aumentaria o estoque (`quantidade_estoque = quantidade_estoque
-- - v_quantidade`, subtrair um negativo soma) se algo chamasse a function
-- direto via RPC.
--
-- Corpo idêntico a 20260811000001_promissoria_dentro_de_misto.sql, só
-- acrescentando a validação de quantidade no loop que já valida
-- preco_unitario. Segue a lição registrada em 2026-08-10 (DECISIONS.md):
-- drop explícito da assinatura antiga antes do create or replace sempre que
-- esta function for tocada, mesmo a lista de parâmetros não mudando aqui —
-- mais seguro replicar a disciplina sempre.
--
-- ROLLBACK:
...
-- (recriar a partir de 20260811000001_promissoria_dentro_de_misto.sql se precisar reverter)
...
declare
  ...
  v_valor_esperado_parcelas numeric(10, 2);
  ...
  v_forma_parcela public.forma_pagamento;
begin
  ...
  if p_status = 'faturado' and v_numero_parcelas > 0 then
    ...
    -- Misto: as parcelas representam só a perna promissória, não o pedido
    -- inteiro (o resto já foi recebido nas outras formas). Fora do misto,
    -- continua sendo o total inteiro (promissória pura ou cartão parcelado).
    if p_forma_pagamento = 'misto' then
      select coalesce(sum((p ->> 'valor')::numeric), 0)
        into v_valor_esperado_parcelas
        from jsonb_array_elements(p_pagamentos_mistos) p
        where (p ->> 'forma') = 'promissoria';
    else
      v_valor_esperado_parcelas := v_total;
    end if;

    if abs(v_soma_parcelas - v_valor_esperado_parcelas) > 0.01 then
      raise exception 'A soma das parcelas (%) não bate com o valor esperado (%).',
        v_soma_parcelas, v_valor_esperado_parcelas;
    end if;
  end if;
  ...
  if p_status = 'faturado' and v_numero_parcelas > 0 then
    -- Misto: a conta a receber é sempre da perna promissória, então grava
    -- 'promissoria' mesmo com forma_pagamento do pedido sendo 'misto' —
    -- gravar 'misto' aqui não diria nada sobre qual das pernas gerou a
    -- dívida.
    v_forma_parcela := case when p_forma_pagamento = 'misto' then 'promissoria'::public.forma_pagamento else p_forma_pagamento end;
    ...
    loop
      ...
      values (
        ...
        v_forma_parcela, v_indice, v_numero_parcelas
      );
    end loop;
  end if;
